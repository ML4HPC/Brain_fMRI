...
def train(model, epoch, train_loader, valid_loader, test_loader, optimizer, loss, output_dir, metric=r2_score, checkpoint_epoch=0, devices):
    model.train()
    ...
    loss.to('cuda:'+str(devices[-1]))
    loss_hist.to('cuda:'+str(devices[-1]))
    ...
    for i in range(start_epoch, epoch):
        ...
        for batch_idx, (batch_img, batch_target) in enumerate(train_loader):            
            ...
            batch_img = batch_img.unsqueeze(1).to('cuda:'+str(devices[0]))

            optimizer.zero_grad()
            cur_loss = 0 
            
            outputs = model(batch_img).squeeze()
            batch_target = batch_target.squeeze().float().to('cuda:'+str(devices[-1]))
        ...
        cur_score= eval(model, valid_loader, metric, devices)
        test_score = eval(model, test_loader, metric, devices)
...
def eval(model, valid_loader, metric, save=False, output_dir=None, valid_type=None, devices):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for batch_idx, (batch_img, batch_target) in enumerate(valid_loader):
            ...
            batch_img = batch_img.unsqueeze(1).float().to('cuda:'+str(devices[0]))
