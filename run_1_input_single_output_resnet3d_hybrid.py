...
#    torch.cuda.set_device(0)
    ...
    devices = [((rank * local_size) + i) %8 for i in range(local_size)]
    
    if args.mri_type == 'T1' or args.mri_type == 'T2':
        model = multi_input_resnet3d.one_struct_input_single_output_resnet3D50(devices, output_classes=1)
    else:
        model = multi_input_resnet3d.one_dti_input_single_output_resnet3D50(devices, output_classes=1)
    ...
    if not args.checkpoint_state:
        train(model, args.epoch, train_loader, valid_loader, test_loader, optimizer, loss, args.output_dir, devices)
    else:
        train(model, args.epoch, train_loader, valid_loader, test_loader, optimizer, loss, args.output_dir, args.checkpoint_epoch, devices)
